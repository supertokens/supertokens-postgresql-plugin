...
- Companion to core's per-storage active-user counting: the last-active fold now uses a single
  `app_id_to_user_id` residency guard (dropping the redundant `apps` guard it subsumes), and the rollup
  prunes `user_last_active` rows whose user has no local `app_id_to_user_id` mapping, so legacy rows that
  earlier landed on the wrong storage stop being double-counted by the summed per-storage MAU read.
