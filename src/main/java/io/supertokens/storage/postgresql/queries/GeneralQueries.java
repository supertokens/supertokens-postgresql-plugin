...
public class GeneralQueries {
        ...
        while (retry) {
            ...
            try {
                ...
                if (doesTableExists(existingTables, Config.getConfig(start).getRecipeUserTenantsTable())) {
                    ...
                    // Sargable-search expression indexes for the dashboard user search arms. Additive and
                    // idempotent; on large deployments operators should pre-create them with CREATE INDEX
                    // CONCURRENTLY before upgrading so this startup DDL is a no-op (see CHANGELOG).
                    backfillIndexDdl.add(
                            AccountInfoQueries.getQueryToCreateSearchValueIndexForRecipeUserTenantsTable(start));
                    backfillIndexDdl.add(
                ...
                if (!doesTableExists(existingTables, Config.getConfig(start).getRecipeUserTenantsTable())) {
                    ...
                    ddl.add(AccountInfoQueries.getQueryToCreateSearchValueIndexForRecipeUserTenantsTable(start));
    ...
    private static long getUsersCount_legacy(Start start, TenantIdentifier tenantIdentifier, RECIPE_ID[] includeRecipeIds)
            throws SQLException, StorageQueryException {
        StringBuilder QUERY = new StringBuilder(
                "SELECT COUNT(*) AS total FROM (");
        QUERY.append("SELECT primary_or_recipe_user_id FROM " + getConfig(start).getUsersTable());
    ...
    // Appends one index-sargable, case-insensitive prefix match on a recipe_user_tenants alias's
    // account_info_value, and binds the raw search term (the SQL appends the wildcard). lower() on both
    // sides preserves the previous ILIKE case-insensitivity — relevant for third-party values whose
    // user-id part is not normalized — while turning the arm into a plain LIKE prefix that the
    // idx_recipe_user_tenants_search_value expression index can seek instead of scanning the tenant.
    private static void appendValuePrefixMatch(StringBuilder query, ArrayList<String> queryParams,
                                               String alias, String term) {
        query.append(" lower(").append(alias).append(".account_info_value) LIKE lower(?) || '%'");
        queryParams.add(term);
    }

    // Appends the email search arm: a prefix match on the whole value (local-part search, backed by
    // idx_recipe_user_tenants_search_value) OR a prefix match on the domain (backed by
    // idx_recipe_user_tenants_search_domain). The domain arm replaces the old non-sargable
    // `account_info_value ILIKE '%@term%'`: for values with exactly one '@' (guaranteed by email
    // normalization at write time) "domain starts with term" is equivalent to "'@term' appears", so
    // this is result-identical on normalized data while becoming a B-tree range seek.
            ...
            if (!hasEmails && !hasPhones && !hasProviders) {
                ...
                if (hasEmails && hasPhones) {
                    ...
                    for (int i = 0; i < dashboardSearchTags.phoneNumbers.size(); i++) {
                        if (i > 0) query.append(" OR");
                        appendValuePrefixMatch(query, queryParams, "rut_phone", dashboardSearchTags.phoneNumbers.get(i));
                    }
                    query.append(")");
                    ...
                    if (hasProviders) {
                        ...
                        for (int i = 0; i < dashboardSearchTags.providers.size(); i++) {
                            if (i > 0) query.append(" OR");
                            appendValuePrefixMatch(query, queryParams, "rut_tp", dashboardSearchTags.providers.get(i));
                        }
                        query.append(")");
                    }
                    ...
                    for (int i = 0; i < dashboardSearchTags.phoneNumbers.size(); i++) {
                        if (i > 0) query.append(" OR");
                        appendValuePrefixMatch(query, queryParams, "rut", dashboardSearchTags.phoneNumbers.get(i));
                    }
                    query.append(") AND rut_tp.account_info_type = 'tparty' AND (");
                    for (int i = 0; i < dashboardSearchTags.providers.size(); i++) {
                        if (i > 0) query.append(" OR");
                    ...
                    for (int i = 0; i < dashboardSearchTags.providers.size(); i++) {
                        if (i > 0) query.append(" OR");
                        appendValuePrefixMatch(query, queryParams, "rut", dashboardSearchTags.providers.get(i));
                    }
                    query.append(")");
                }
    ...
    public static AuthRecipeUserInfo[] listPrimaryUsersByThirdPartyInfo(Start start,
        ...
        List<String> userIds;
        if (Config.getConfig(start).getMigrationMode().readsFromNewTables()) {
            userIds = AccountInfoQueries.listPrimaryUserIdsByThirdPartyInfo(start, appIdentifier,
                    thirdPartyId, thirdPartyUserId);
        } else {
            userIds = ThirdPartyQueries.listPrimaryUserIdsByThirdPartyInfo_legacy(start, appIdentifier,
                    thirdPartyId, thirdPartyUserId);
        }
    ...
    public static AuthRecipeUserInfo[] listPrimaryUsersByThirdPartyInfo_Transaction(Start start, Connection sqlCon,
        ...
        List<String> userIds;
        if (Config.getConfig(start).getMigrationMode().readsFromNewTables()) {
            userIds = AccountInfoQueries.listPrimaryUserIdsByThirdPartyInfo_Transaction(start, sqlCon,
                    appIdentifier, thirdPartyId, thirdPartyUserId);
        } else {
            userIds = ThirdPartyQueries.listPrimaryUserIdsByThirdPartyInfo_legacy_Transaction(start, sqlCon,
                    appIdentifier, thirdPartyId, thirdPartyUserId);
        }
    ...
    public static AuthRecipeUserInfo[] listPrimaryUsersByEmail_legacy_forApp(Start start, AppIdentifier appIdentifier,
                                                                        String email)
            throws StorageQueryException, SQLException {
        // App-scoped email lookup across all recipe tables (no tenant filter).
        // Used for email conflict checks where cross-tenant linked accounts must be considered.
        ...
        List<String> epUserIds = execute(start, epQuery, pst -> {
            pst.setString(1, appIdentifier.getAppId());
        ...
        List<String> tpUserIds = execute(start, tpQuery, pst -> {
            pst.setString(1, appIdentifier.getAppId());
            pst.setString(2, email);
        }, result -> {
            List<String> ids = new ArrayList<>();
        ...
        List<String> plUserIds = execute(start, plQuery, pst -> {
            pst.setString(1, appIdentifier.getAppId());
            pst.setString(2, email);
        }, result -> {
            List<String> ids = new ArrayList<>();
        ...
        List<String> waUserIds = execute(start, waQuery, pst -> {
            pst.setString(1, appIdentifier.getAppId());
            pst.setString(2, email);
        }, result -> {
            List<String> ids = new ArrayList<>();
    ...
    public static AuthRecipeUserInfo[] listPrimaryUsersByPhoneNumber_legacy_forApp(Start start,
        ...
        List<String> userIds = execute(start, plQuery, pst -> {
            pst.setString(1, appIdentifier.getAppId());
    ...
    public static AuthRecipeUserInfo[] listPrimaryUsersByEmail_legacy(Start start, TenantIdentifier tenantIdentifier,
                                                                        String email)
            throws StorageQueryException, SQLException {
        List<String> userIds = new ArrayList<>();
    ...
    private static List<AuthRecipeUserInfo> getPrimaryUserInfoForUserIds_Transaction(Start start, Connection sqlCon,
        ...
        String tenantJoin;
        if (Config.getConfig(start).getMigrationMode().readsFromNewTables()) {
            tenantJoin = "    LEFT JOIN " + getConfig(start).getRecipeUserTenantsTable() +
                    "    as rt ON au.app_id = rt.app_id AND au.user_id = rt.recipe_user_id";
        } else {
            tenantJoin = "    LEFT JOIN " + getConfig(start).getUsersTable() +
                    "    as rt ON au.app_id = rt.app_id AND au.user_id = rt.user_id";
        }
        String QUERY =
                "SELECT au.user_id, au.primary_or_recipe_user_id, au.is_linked_or_is_a_primary_user, au.recipe_id, " +
                        "au.time_joined, rt.tenant_id " +
    ...
    public static Map<String, List<String>> getTenantIdsForUserIds_transaction(Start start, Connection sqlCon,
        ...
        if (userIds != null && userIds.length > 0) {
            ...
            String userIdColumn;
            if (Config.getConfig(start).getMigrationMode().readsFromNewTables()) {
                table = getConfig(start).getRecipeUserTenantsTable();
                userIdColumn = "recipe_user_id";
            } else {
                table = getConfig(start).getUsersTable();
                userIdColumn = "user_id";
            }
            StringBuilder QUERY = new StringBuilder("SELECT DISTINCT " + userIdColumn + " AS user_id, tenant_id "
                    + "FROM " + table);
            QUERY.append(" WHERE " + userIdColumn + " IN (");
