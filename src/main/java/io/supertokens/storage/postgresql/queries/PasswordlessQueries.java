...
public class PasswordlessQueries {
    ...
    public static String getQueryToCreateUsersTable(Start start) {
        ...
        return "CREATE TABLE IF NOT EXISTS " + usersTable + " ("
                ...
                + "CONSTRAINT " + Utils.getConstraintName(schema, usersTable, "email", "key")
                + " UNIQUE (app_id, email),"
                + "CONSTRAINT " + Utils.getConstraintName(schema, usersTable, "phone_number", "key")
    ...
    public static String getQueryToCreateDevicesTable(Start start) {
        ...
        return "CREATE TABLE IF NOT EXISTS " + devicesTable + " ("
                ...
                + "CONSTRAINT " + Utils.getConstraintName(schema, devicesTable, "app_id", "fkey")
                + " FOREIGN KEY(app_id, tenant_id)"
                + " REFERENCES " + Config.getConfig(start).getTenantsTable() +  "(app_id, tenant_id) ON DELETE CASCADE"
                + ");";
        // TODO constraint to tenants table
        // @formatter:on
    }
    ...
    public static String getQueryToCreateCodesTable(Start start) {
        ...
        return "CREATE TABLE IF NOT EXISTS " + codesTable + " ("
                ...
                + " REFERENCES " + Config.getConfig(start).getPasswordlessDevicesTable() + "(app_id, tenant_id, device_id_hash) ON DELETE CASCADE ON UPDATE CASCADE"
                + ");";
        // @formatter:on
    }
    ...
    private static void createCode_Transaction(Start start, Connection con, TenantIdentifier tenantIdentifier, PasswordlessCode code)
        ...
        String QUERY = "INSERT INTO " + getConfig(start).getPasswordlessCodesTable()
                + "(app_id, tenant_id, code_id, device_id_hash, link_code_hash, created_at)" + " VALUES(?, ?, ?, ?, ?, ?)";
        update(con, QUERY, pst -> {
            pst.setString(1, tenantIdentifier.getAppId());
            pst.setString(2, tenantIdentifier.getTenantId());
            ...
            try {
                {
                    String QUERY = "INSERT INTO " + getConfig(start).getAppIdToUserIdTable()
                            + "(app_id, user_id, recipe_id, time_joined)"
                            + " VALUES(?, ?, ?, ?)";
                    update(sqlCon, QUERY, pst -> {
                        pst.setString(1, tenantIdentifier.getAppId());
...

                {
                    String QUERY = "INSERT INTO " + getConfig(start).getPasswordlessUsersTable()
                            + "(app_id, user_id, email, phone_number, time_joined)"
                            + " VALUES(?, ?, ?, ?, ?)";
                    update(sqlCon, QUERY, pst -> {
                        pst.setString(1, tenantIdentifier.getAppId());
                        pst.setString(2, user.id);
            ...
            try {
                ...
                UserInfo user;
                {
                    String QUERY = "DELETE FROM " + Config.getConfig(start).getPasswordlessUsersTable()
                            + " WHERE app_id = ? AND user_id = ? RETURNING user_id, email, phone_number, time_joined";

                    user = execute(sqlCon, QUERY, pst -> {
                        pst.setString(1, appIdentifier.getAppId());
                        pst.setString(2, userId);
                    }, result -> {
                        if (result.next()) {
                ...
                if (user != null) {
                    if (user.email != null) {
                        deleteDevicesByEmail_Transaction(start, sqlCon, appIdentifier, user.email);
                    }
                    if (user.phoneNumber != null) {
                        deleteDevicesByPhoneNumber_Transaction(start, sqlCon, appIdentifier, user.phoneNumber);
                    }
                }
    ...
    public static PasswordlessDevice[] getDevicesByPhoneNumber(Start start, TenantIdentifier tenantIdentifier, @Nonnull String phoneNumber)
            throws StorageQueryException, SQLException {
        String QUERY = "SELECT device_id_hash, email, phone_number, link_code_salt, failed_attempts FROM "
                + getConfig(start).getPasswordlessDevicesTable()
                + " WHERE app_id = ? AND tenant_id = ? AND phone_number = ?";
