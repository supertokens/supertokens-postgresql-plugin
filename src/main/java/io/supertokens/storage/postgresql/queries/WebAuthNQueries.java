...
public class WebAuthNQueries {
    ...
    static String getQueryToCreateWebAuthNUsersTable(Start start){
        ...
        String webAuthNUsersTableName = getConfig(start).getWebAuthNUsersTable();
        return  "CREATE TABLE IF NOT EXISTS " + webAuthNUsersTableName + "(" +
                ...
                " FOREIGN KEY (app_id, user_id) REFERENCES " + getConfig(start).getAppIdToUserIdTable() +
    ...
    static String getQueryToCreateWebAuthNUsersToTenantTable(Start start){
        String schema = getConfig(start).getTableSchema();
        String webAuthNUserToTenantTableName = getConfig(start).getWebAuthNUserToTenantTable();
        return  "CREATE TABLE IF NOT EXISTS  " + webAuthNUserToTenantTableName +" (" +
                ...
                " REFERENCES "+ getConfig(start).getUsersTable()+" (app_id, tenant_id, user_id) ON DELETE CASCADE" +
                ");";
    }
    ...
    static String getQueryToCreateWebAuthNGeneratedOptionsTable(Start start){
        ...
        return  "CREATE TABLE IF NOT EXISTS " + webAuthNGeneratedOptionsTable + "(" +
                ...
                "  REFERENCES " + getConfig(start).getTenantsTable() + " (app_id, tenant_id) ON DELETE CASCADE" +
                ");";
    }
    ...
    static String getQueryToCreateWebAuthNCredentialsTable(Start start){
        String schema = getConfig(start).getTableSchema();
        String webAuthNCredentialsTable = getConfig(start).getWebAuthNCredentialsTable();
        return  "CREATE TABLE IF NOT EXISTS "+ webAuthNCredentialsTable + "(" +
                ...
                getConfig(start).getWebAuthNUsersTable() + " (app_id, user_id) ON DELETE CASCADE" +
                ");";
    }
    ...
    public static String getQueryToCreateWebAuthNAccountRecoveryTokenTable(Start start) {
        ...
        return "CREATE TABLE IF NOT EXISTS " + getConfig(start).getWebAuthNAccountRecoveryTokenTable() + "(" +
                ...
                getConfig(start).getAppIdToUserIdTable() + " (app_id, user_id) ON DELETE CASCADE ON UPDATE CASCADE" +
                ");";
    }
    ...
    public static WebAuthNOptions loadOptionsById(Start start, TenantIdentifier tenantIdentifier, String optionsId)
            throws SQLException, StorageQueryException {
        String QUERY = "SELECT * FROM " + getConfig(start).getWebAuthNGeneratedOptionsTable()
    ...
    public static WebAuthNStoredCredential loadCredentialByIdForUser(Start start, TenantIdentifier tenantIdentifier, String credentialId, String recipeUserId)
        ...
        String QUERY = "SELECT * FROM " + getConfig(start).getWebAuthNCredentialsTable()
        ...
        try {
            ...
            if (mode.writesToOldTables()) { // webauthn_user_to_tenant
                String insertWebauthNUsersToTenant =
                        "INSERT INTO " + getConfig(start).getWebAuthNUserToTenantTable()
            ...
            // webauthn_users
            String insertWebauthNUsers = "INSERT INTO " + getConfig(start).getWebAuthNUsersTable()
    ...
    public static List<String> getPrimaryUserIdsUsingEmails_Transaction(Start start, Connection sqlConnection,
        ...
        if (Config.getConfig(start).getMigrationMode().readsFromNewTables()) {
            return getPrimaryUserIdsUsingEmails_Transaction_new(start, sqlConnection, appIdentifier, emails);
        }
        return getPrimaryUserIdsUsingEmails_Transaction_legacy(start, sqlConnection, appIdentifier, emails);
    }

    private static List<String> getPrimaryUserIdsUsingEmails_Transaction_legacy(Start start, Connection sqlConnection,
                                                                                 AppIdentifier appIdentifier, List<String> emails)
            throws SQLException, StorageQueryException {
        String QUERY = "SELECT DISTINCT all_users.primary_or_recipe_user_id AS user_id "
    ...
    private static List<String> getPrimaryUserIdsUsingEmails_Transaction_new(Start start, Connection sqlConnection,
                                                                              AppIdentifier appIdentifier, List<String> emails)
            throws SQLException, StorageQueryException {
        String QUERY = "SELECT DISTINCT auid.primary_or_recipe_user_id AS user_id "
                + "FROM " + getConfig(start).getRecipeUserAccountInfosTable() + " AS ruai"
    ...
    public static Collection<? extends LoginMethod> getUsersInfoUsingIdList_Transaction(Start start, Connection connection, Set<String> ids, AppIdentifier appIdentifier)
        ...
        //   tenant_id, account_info_type). For webauthn there's only an 'email' row per tenant.
        //
        // In MIGRATED mode, all_auth_recipe_users isn't written to, so the legacy JOIN returns
        // zero rows and getAuthRecipeUserInfo returns null — which cascades into NPEs at every
        // webauthn call site. Read the source-of-truth table for whichever mode we're in.
    ...
    public static void updateCounter_Transaction(Start start, Connection sqlCon, TenantIdentifier tenantIdentifier, String credentialId, long counter)
            throws SQLException, StorageQueryException {
        String UPDATE = "UPDATE " + getConfig(start).getWebAuthNCredentialsTable()
    ...
    public static List<WebAuthNStoredCredential> listCredentials(Start start, TenantIdentifier tenantIdentifier,
        ...
        String LIST_QUERY =  "SELECT * FROM " + getConfig(start).getWebAuthNCredentialsTable() +
    ...
    public static AccountRecoveryTokenInfo getAccountRecoveryTokenInfoByToken_Transaction(Start start, TenantIdentifier tenantIdentifier, Connection con, String token)
            throws SQLException, StorageQueryException {
        String QUERY = "SELECT * FROM " + getConfig(start).getWebAuthNAccountRecoveryTokenTable() + " WHERE app_id = ? AND tenant_id = ? AND token = ?";
    ...
    public static void deleteAccountRecoveryTokenByEmail_Transaction(Start start, Connection con, TenantIdentifier tenantIdentifier, String email)
            throws SQLException, StorageQueryException {
        String DELETE = "DELETE FROM " + getConfig(start).getWebAuthNAccountRecoveryTokenTable() + " WHERE app_id = ? AND tenant_id = ? AND email = ?";
