...
public class UserLockingQueries {
    ...
    public static List<LockedUser> lockUsers(Start start, Connection con, AppIdentifier appIdentifier,
        ...
        // Step 1: Read primary user mappings for all users (without lock)
        ...
        // Step 2: Lock all users in consistent alphabetical order (prevents deadlocks)
        for (String id : allIdsToLock) {
            lockSingleUser(start, con, appIdentifier, id);
        }

        // Step 3: Re-read primary mappings under lock (may have changed)
    ...
    /**
     * Acquires FOR UPDATE lock on a single user row.
